...
package com.lumencloud.lumen.common.core.util;
...
public class WebUtils extends org.springframework.web.util.WebUtils {
	...
	public String getClientId(ServerHttpRequest request) {
		...
		if (clientId == null) {
			clientId = request.getQueryParams().getFirst("clientId");
		}
		if (clientId == null) {
			throw new CheckedException("璇锋眰涓璫lient淇℃伅涓虹┖");
		}
		return clientId;
	}
	...
	public String getClientId(HttpServletRequest request) {
		return findClientId(request).orElseThrow(() -> new CheckedException("璇锋眰涓璫lient淇℃伅涓虹┖"));
	}

	public Optional<String> findClientId(HttpServletRequest request) {
		String header = request.getHeader(HttpHeaders.AUTHORIZATION);
		if (header != null && header.startsWith(BASIC_)) {
			return Optional.of(splitClient(header)[0]);
		}
	...
	public String getClientId() {
		if (WebUtils.getRequest().isPresent()) {
			return findClientId().orElseThrow(() -> new CheckedException("璇锋眰涓璫lient淇℃伅涓虹┖"));
		}
		return null;
	}

	public Optional<String> findClientId() {
		return getRequest().flatMap(WebUtils::findClientId);
	}
